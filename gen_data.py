...
class RC():
    ...
    def run(self):
        ...
        if self.method == 'RLS':
            return self.train_and_test_RLS(self.rx_time_cp, tx_label_time_CP)
    ...
    def train_and_test_RLS(self, rx_time_cp, tx_label_time_cp, train_with_CP=True):
        ...
        return esn_input_windowed, esn_label, test_input_windowed, self.tx_freq[self.N_sync_frame+1:, :] 
    
    
    def my_test(self, predict_time):
        predicts_freq = self.esn_output_to_block_f(predict_time, self.N_total_frame, cp_removed=False, remove_delay=True)
        ber_ESN = self.ber(predicts_freq[self.N_sync_frame+1:, :], self.tx_freq[self.N_sync_frame+1:, :])
        #ber_ESN = self.ber(self.tx_freq[self.N_sync_frame+1:, :], self.tx_freq[self.N_sync_frame+1:, :])
        return ber_ESN
    
    def time_to_freq(self, predict_time, N_total_frame, remove_delay=True):
        predicts_freq = self.esn_output_to_block_f(predict_time, N_total_frame, cp_removed=True, remove_delay=remove_delay)
        return predicts_freq

    def my_new_test(self, pred, target):
        return self.ber(pred, target)
    ...
    def ber(self, a, b):
        ...
        #print("pred values in ber", a_list[0, 50 : 70])
        #print("real values in ber", b_list[0, 50 : 70])
